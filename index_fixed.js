        ...
        function drawMatrix() {
            ...
            for (let i = 0; i < drops.length; i++) {
                ...
                if (headY - (trailLength * fontSize) > canvas.height && Math.random() > 0.98) {
                    ...
                    // 重新投胎时，再次赋予全新的随机速度，让交错感永不停滞
                    speeds[i] = Math.random() * 0.5 + 0.35; 
                }
                
                // 【核心修正】：每一列按照自己独立的速度向下位移，产生完美的交错纵深感
                drops[i] += speeds[i]; 
            }
        }
        
        // 整体基础帧率踩刹车，控制在 30ms，优雅、丝滑、不晃眼
